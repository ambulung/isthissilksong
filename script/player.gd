...
func perform_down_attack():
    ...
    # Force downward direction (so attack rotates to 90 degrees)
    if attack_instance.has_method("set_attack_direction"):
        attack_instance.set_attack_direction(0.0)

    get_parent().add_child(attack_instance)

    # Connect hit signal for damage/bounce
...
func update_animations():
    ...
    # --- NEW: Flip player sprite horizontally using scale.x ---
    # Get the current Y scale to preserve it if it's not 1.0 (e.g., if resized in editor)
    var current_scale_y = abs(animated_sprite.scale.y) # Use abs to ensure it's always positive here
    
    if last_direction_x > 0: # Facing right
        animated_sprite.scale = Vector2(1.0, current_scale_y)
    else: # Facing left (last_direction_x < 0)
        animated_sprite.scale = Vector2(-1.0, current_scale_y)
    # --- END NEW: Player Sprite Flip ---
